 ...
 */
function computeFailureProbability(record: {
    engine_rpm: number;
    ...
    const rpmRisk = Math.min(record.engine_rpm / 5000, 1.0);
    const oilRisk = Math.max(0, 1 - record.lub_oil_pressure / 5.0);
    const fuelRisk = Math.max(0, 1 - record.fuel_pressure / 60.0);
    const coolantPRisk = Math.max(0, 1 - record.coolant_pressure / 35.0);
    const oilTempRisk = Math.min(record.lub_oil_temp / 140.0, 1.0);
    const coolantTempRisk = Math.min(record.coolant_temp / 120.0, 1.0);
    const brakeRisk = record.brake_wear_pct / 100.0;
    const engineRisk = record.engine_condition === 1 ? 0 : 1;
...
async function main() {
    ...
    for (const v of vehicles) {
        ...
        console.log(`  ✓ ${v.vehicle_id} (${v.make} ${v.model}) — failure_prob: ${failProb}%`);
    }
